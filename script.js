...
let rider = { x: 50, y: groundY, width: 60, height: 40, vy: 0, jumping: false };
let fireParticles = [];
let obstacles = [];
let score = 0;
...
function drawRider() { ctx.fillStyle = "#333"; ctx.fillRect(rider.x, rider.y, rider.width, rider.height); }
function drawObstacle(obs) { ctx.fillStyle = obs.type === 'car' ? "blue" : "gray"; ctx.fillRect(obs.x, obs.y, obs.width, obs.height); }
function checkCollision(obs) {
  return rider.x < obs.x + obs.width && rider.x + rider.width > obs.x && rider.y < obs.y + obs.height && rider.y + rider.height > obs.y;
}

function restartGame() {
  obstacles = []; fireParticles = []; score = 0; speed = 0; obstacleSpeed = 4;
  rider.y = groundY; rider.vy = 0; rider.jumping = false;
  gameOver = false; gameStarted = true; isPaused = false; elapsedTime = 0; startTime = Date.now();
...
function togglePause() {
  ...
  if (!isPaused) { startTime = Date.now() - elapsedTime * 1000; requestAnimationFrame(update); }
}
...
function update() {
  ...
  if (rider.y < 2) { rider.y = 2; rider.vy = 0; }
  if (rider.y >= groundY) { rider.y = groundY; rider.vy = 0; rider.jumping = false; }
  drawRider(); drawFire(); spawnObstacle();
  for (let obs of obstacles) {
    obs.x -= obstacleSpeed; drawObstacle(obs);
  ...
  if (score % 500 === 0 && obstacleSpeed < 8) obstacleSpeed += 0.1;
  score++; if (speed < 777) speed++;
